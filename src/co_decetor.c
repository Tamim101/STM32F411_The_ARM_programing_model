 ...
 *  Bare metal, no HAL.  FIXED VERSION.
 *
 ...
 *    1. Calibration used to fail SILENTLY and fall back to a
 *       fixed R0_DEFAULT with no warning. That bad R0 got saved
 *       to flash and reused forever -> PPM math stayed crushed
 *       near zero even with real CO present -> alarm never fired.
 *       Now: 3 retries, clear on-screen + beep failure warning,
 *       and the resulting R0 is shown on the LCD after calibration
 *       so you can sanity-check it.
 *    2. Removed a stray dead statement in voltage_to_rs().
 *    3. Added an optional DEBUG screen (toggles automatically)
 *       showing live Rs and R0 so you can watch the sensor
 *       actually respond to gas and tune thresholds with real data.
 *
 *  WIRING
 ...
 *      away from the generator) for calibration.
 *    - Brand-new MQ-7 sensors ideally want 24-48h of continuous
 *      power (burn-in) before their readings are trustworthy.
 *      A 60s warm-up is enough to stabilise a sensor that has
 *      already been burned in, not a sensor fresh out of the bag.
 *    - It warms up, calibrates, and saves R0 to flash.
 *    - To force a fresh calibration later, change CALIB_MAGIC below.
 *
 *  DO NOT test this by running the generator indoors on purpose —
 *  that recreates the exact hazard you're trying to detect. Test
 *  outdoors, or from a safe distance from an already-running unit,
 *  and watch the DEBUG screen to confirm Rs is actually dropping.
 * ============================================================
 */
...
#define CALIB_MAGIC      0xCAFE5003UL   /* bumped -> forces one fresh recalibration */
...
#define R0_DEFAULT       1000.0f        /* only used if calibration fails 3x */
#define R0_MIN_VALID     50.0f
#define R0_MAX_VALID     50000.0f

/* Alarm thresholds. Reference points (approximate, general guidance,
 * not medical advice): ASHRAE recommends indoor CO stay under ~9ppm;
 * OSHA 8-hr workplace limit is 35ppm; above ~200ppm symptoms appear
 * within a couple of hours. For an early-warning hobby monitor near
 * a generator, alarming well before 200ppm gives you more time.
 * Current values below are intentionally sensitive for bench testing;
 * once you've confirmed with the DEBUG screen that Rs responds
 * correctly to real CO, pick alarm/clear numbers that make sense
 * for your situation. */
 ...
 * If this is a brand-new sensor, let it run powered (in clean air) for
 * a day or more before you trust CALIB, then bump CALIB_MAGIC to
 * force a fresh calibration after that burn-in period. */
...
/* Set to 1 to alternate the normal screen with a raw Rs/R0 debug
 * screen every few seconds. Very useful for confirming the sensor
 * is actually responding to gas before you trust the alarm. */
#define DEBUG_MODE       1
#define DEBUG_PERIOD_S   4U
 ...
 * Retries up to 3 times. If it never gets enough valid samples,
 * it clearly WARNS you on the LCD and with a distinct beep pattern
 * instead of silently saving a guessed R0. On success it shows the
 * resulting R0 on screen so you can sanity-check it.
 * Returns 1 on real calibration success, 0 if it fell back to default. */
static uint8_t do_calibration(void) {
    warm_up();
    ...
    for(uint8_t attempt = 0; attempt < 3; attempt++) {
        ...
        if(count >= 50) {
            float r0 = rs_sum / (float)count / MQ7_CLEAN_AIR_FACTOR;
            if(r0 >= R0_MIN_VALID && r0 <= R0_MAX_VALID) {
                g_R0 = r0;
                flash_save_r0(g_R0);
                lcd_clr();
                lcd_pos(0, 0); lcd_str("  CALIB OK      ");
                lcd_pos(0, 1); lcd_str("R0="); lcd_num((uint32_t)g_R0, 6); lcd_str(" ohm");
                delay_ms(3000);
                return 1;
            }
        }
        ...
        lcd_clr();
        lcd_pos(0, 0); lcd_str(" CALIB RETRY ");
        lcd_chr((uint8_t)('1' + attempt)); lcd_str("/3 ");
        beep(2);
        delay_ms(1000);
    }

    /* All attempts failed: warn clearly instead of failing silently. */
    g_R0 = R0_DEFAULT;
    lcd_clr();
    lcd_pos(0, 0); lcd_str(" CALIB FAILED!  ");
    lcd_pos(0, 1); lcd_str(" CHECK WIRING   ");
    beep(4);
    delay_ms(3000);
    return 0;
}
...
/* Raw diagnostic screen: lets you WATCH Rs move as gas changes,
 * instead of trusting the ppm number blindly. */
static void screen_debug(float rs, float co) {
    uint32_t rsv = (uint32_t)(rs > 999999.0f ? 999999.0f : rs);
    uint32_t r0v = (uint32_t)(g_R0 > 99999.0f ? 99999.0f : g_R0);
    uint32_t cov = (uint32_t)(co > 999.0f ? 999.0f : co);
    lcd_pos(0, 0); lcd_str("Rs:"); lcd_num(rsv, 6); lcd_str("      ");
    lcd_pos(0, 1); lcd_str("R0:"); lcd_num(r0v, 5); lcd_str(" PPM:"); lcd_num(cov, 3);
}
...
int main(void) {
    ...
    if(saved > R0_MIN_VALID && saved < R0_MAX_VALID) {
        ...
        do_calibration();
        lcd_clr();
        lcd_pos(0, 0); lcd_str("  MONITORING... ");
        beep(3);
    ...
    uint8_t debug_showing = 0;
    uint32_t debug_t = g_ms;
    screen_normal(co);
    ...
    while(1) {
        if((g_ms - lcd_t) >= 1000U) {       /* read once per second */
            ...
            if(!alarmed) {
                if(co >= CO_ALARM_PPM) {
                    ...
                    debug_showing = 0;
                } else if(!DEBUG_MODE || !debug_showing) {
                    screen_normal(co);
                }
            } else {
                if(co < CO_CLEAR_PPM) {
        ...
        /* Debug screen only shown while not alarmed, so the alarm
         * message is never hidden by diagnostics. */
        if(DEBUG_MODE && !alarmed && (g_ms - debug_t) >= (DEBUG_PERIOD_S * 1000U)) {
            debug_t = g_ms;
            debug_showing = !debug_showing;
            lcd_clr();
            if(debug_showing) screen_debug(g_last_rs, co);
            else screen_normal(co);
        }
